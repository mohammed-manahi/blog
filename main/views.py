...
from main.forms import EmailPostForm, CommentForm, SearchForm
...
def post_search(request):
    # Create advanced search using postgres search
    form = SearchForm()
    query = None
    results = []
    if "query" in request.GET:
        ...
        if form.is_valid():
            ...
            search_query = SearchQuery(query, "english")
            # Annotate search fields title and body and filter based on search query
            results = Post.published.annotate(search=search_vector, rank=SearchRank(search_vector, search_query)) \
                .filter(search=search_query).order_by("-rank")
    template = "main/post_search.html"
